...
public enum HOKColorScheme {
    ...
    func getColors() -> HOKColors {
        ...
        case .Asagi:
            return HOKColors(
                backGroundColor: UIColorHex(hex: 0x0bada8),
                buttonColor: UIColorHex(hex: 0x08827e),
                cancelButtonColor: UIColorHex(hex: 0x6dcecb),
                fontColor: UIColorHex(hex: 0xffffff)
            )
        case .Matcha:
            return HOKColors(
                backGroundColor: UIColorHex(hex: 0x314631),
                buttonColor: UIColorHex(hex: 0x618c61),
                cancelButtonColor: UIColorHex(hex: 0x496949),
                fontColor: UIColorHex(hex: 0xffffff)
            )
        case .Tsubaki:
            return HOKColors(
                backGroundColor: UIColorHex(hex: 0xe5384c),
                buttonColor: UIColorHex(hex: 0xac2a39),
                cancelButtonColor: UIColorHex(hex: 0xc75764),
                fontColor: UIColorHex(hex: 0xffffff)
            )
        case .Inari:
            return HOKColors(
                backGroundColor: UIColorHex(hex: 0xdd4d05),
                buttonColor: UIColorHex(hex: 0xa63a04),
                cancelButtonColor: UIColorHex(hex: 0xb24312),
                fontColor: UIColorHex(hex: 0x231e1c)
            )
        case .Karasu:
            return HOKColors(
                backGroundColor: UIColorHex(hex: 0x180614),
                buttonColor: UIColorHex(hex: 0x3d303d),
                cancelButtonColor: UIColorHex(hex: 0x261d26),
                fontColor: UIColorHex(hex: 0x9b9981)
            )
        case .Enshu:
            return HOKColors(
                backGroundColor: UIColorHex(hex: 0xccccbe),
                buttonColor: UIColorHex(hex: 0xffffff),
                cancelButtonColor: UIColorHex(hex: 0xe5e5d8),
                fontColor: UIColorHex(hex: 0x9b9981)
            )
        default: // Hokusai
            return HOKColors(
                backGroundColor: UIColorHex(hex: 0x00AFF0),
                buttonColor: UIColorHex(hex: 0x197EAD),
                cancelButtonColor: UIColorHex(hex: 0x1D94CA),
                fontColor: UIColorHex(hex: 0xffffff)
            )
        }
    }
...
final public class HOKButton: UIButton {
    ...
    func setColor(colors: HOKColors) {
        self.setTitleColor(colors.fontColor, for: .Normal)
...
final public class Hokusai: UIViewController, UIGestureRecognizerDelegate {
    ...
    required public init() {
        ...
        NotificationCenter.defaultCenter.addObserver(self, selector: #selector(Hokusai.onOrientationChange(_:)), name: UIDeviceOrientationDidChangeNotification, object: nil)
    }
    ...
    public func addButton(title:String, action:@escaping ()->Void) -> HOKButton {
    ...
    public func addCancelButton(title:String) -> HOKButton {
        if let cancelButtonAction = cancelButtonAction {
            let btn = addButton(title: title, action: cancelButtonAction)
    ...
    public func show() {
        ...
        menuView.setShapeLayer(colors: colors!)
        
        // Add a cancel button
        self.addCancelButton(title: cancelButtonTitle)
        
        // Add a title label when title is set
        if !headline.isEmpty {
            self.addLabel(text: headline)
        }
        
        // Add a message label when message is set
        if !message.isEmpty {
            self.addMessageLabel(text: message)
        }

        // Style buttons
        for btn in buttons {
            btn.layer.cornerRadius = kButtonHeight * 0.5
            btn.setFontName(fontName: fontName)
            btn.setColor(colors: colors!)
        }
        
        // Style labels
        for label in labels {
            label.setFontName( fontName: label.isTitle ? fontName : lightFontName)
            label.setColor(colors: colors!)
        }
    ...
    func buttonDarker(btn:HOKButton) {
        btn.backgroundColor = btn.backgroundColor!.darkerColorWithPercentage(percent: 0.2)
    }

    // Make the buttons lighter when user release finger.
    func buttonLighter(btn:HOKButton) {
        btn.backgroundColor = btn.backgroundColor!.lighterColorWithPercentage(percent: 0.2)
    }
...
extension UIColor {
    ...
    func lighterColorWithPercentage(percent : Double) -> UIColor {
        return colorWithBrightnessSwiftHokusai.xcworkspace(factor: CGFloat(1 + percent));
    }

    func darkerColorWithPercentage(percent : Double) -> UIColor {
        return colorWithBrightness(factor: CGFloat(1 - percent));
    }
